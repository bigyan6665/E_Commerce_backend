from app.schemas.customerprofile_schema import LoyaltyPointsUpdate, Operation, VerificationUpdate, CustomerProfileResponse
...
router = APIRouter(prefix="/customer_profile")
...
# admin can update any customers' pp
# customers can update own pp only
@router.put("/update_profile_picture/{userid}",response_model=APIResponse[dict])
def update_profile_picture(userid:int,image:UploadFile = File(...),db:Session = Depends(get_db),current_user:dict = Depends(PermissionChecker(allowed_roles=[Roles.admin,Roles.customer]))):
    ...
    if current_user.get("role") != Roles.admin and matched_profile.user_id != current_user.get("id"):
...
# admin can access any customers' profile
# customers can access own profile only
@router.get("/{userid}",response_model=APIResponse[CustomerProfileResponse])
def get_profile(userid:int,db:Session = Depends(get_db),current_user:dict = Depends(PermissionChecker(allowed_roles=[Roles.admin,Roles.customer]))):
    profile = db.query(CustomerProfile).filter(CustomerProfile.user_id == userid).first()
    if profile is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Customer profile not found") 
    
    if current_user.get("role") != Roles.admin and profile.user_id != current_user.get("id"):
       raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail=f"Permission denied") 

    return APIResponse(message="Customer profile fetched succesfully",success=True,content=profile)

# admin can update any customers' loyalty point
# customers can update only own loyalty points
@router.put("/update_loyalty_points/{userid}",response_model=APIResponse[CustomerProfileResponse])
def update_loyalty_points(userid:int,data:LoyaltyPointsUpdate,db:Session = Depends(get_db),current_user:dict = Depends(PermissionChecker(allowed_roles=[Roles.admin,Roles.customer]))):
    profile = db.query(CustomerProfile).filter(CustomerProfile.user_id == userid).first()
    if profile is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Customer profile not found") 
    ...
    try:
        if data.operation == Operation.add:
            ...
            if profile.loyalty_points >= data.points:
                profile.loyalty_points = profile.loyalty_points - data.points
            else:
                raise Exception("This much loyalty points is not available") 
        db.commit()
        db.refresh(profile)
    except Exception as e :
        db.rollback()
        raise HTTPException(detail=f"Error in updating loyalty points: {str(e)}",status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return APIResponse(message="Loyalty point updated successfully",success=True,content=profile)

# admin can update any customers' verification status
# customers can update only own verification status
@router.put("/update_verification_status/{userid}",response_model=APIResponse[CustomerProfileResponse])
def update_verification_status(userid:int,data:VerificationUpdate,db:Session = Depends(get_db),current_user:dict = Depends(PermissionChecker(allowed_roles=[Roles.admin,Roles.customer]))):
    profile = db.query(CustomerProfile).filter(CustomerProfile.user_id == userid).first()
    if profile is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Customer profile not found") 
